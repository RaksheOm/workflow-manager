...
def serialize_transition(transition):
    ...
    return {
        ...
        "guard": {
            "permissions": [expr_to_text(p) for p in getattr(guard, "permissions", ())],
            "roles": [expr_to_text(r) for r in getattr(guard, "roles", ())],
            "groups": [expr_to_text(g) for g in getattr(guard, "groups", ())],
            "expr": expr_to_text(getattr(guard, "expr", "")),
        },
    }
...
class ListTransitions(Service):
    ...
    def reply(self):
        if len(self.params) < 1:
            ...
            return {
                "error": "Invalid URL format. Expected: /@transitions/{workflow_id}"
            }
...
class GetTransition(Service):
    ...
    def reply(self):
        if len(self.params) < 2:
            self.request.response.setStatus(400)
            return {
                "error": "Invalid URL format. Expected: /@transitions/{workflow_id}/{transition_id}"
            }
        ...
        if not base.selected_transition:
            self.request.response.setStatus(404)
            return {
                "error": f"Transition '{transition_id}' in workflow '{workflow_id}' not found."
            }
        ...
        return {
            ...
            "states_with_this_transition": [
                s.id
                for s in base.available_states
                if transition.id in getattr(s, "transitions", ())
            ],
            "available_states": [
                {"id": s.id, "title": s.title} for s in base.available_states
            ],
            "available_transitions": [
                {"id": t.id, "title": t.title} for t in base.available_transitions
            ],
            "guard_options": {
...
class AddTransition(Service):
    ...
    def reply(self):
        ...
        if not title:
            self.request.response.setStatus(400)
            return {
                "error": "A 'title' for the new transition is required in the request body."
            }

        if not re.match(r"^[a-zA-Z_][a-zA-Z0-9_]*$", transition_id):
            self.request.response.setStatus(400)
            return {
                "error": "Invalid transition ID format. Use only letters, numbers, and underscores."
            }
        ...
        try:
            ...
            if "new_state_id" in body:
                ...
                if new_state_id in workflow.states.objectIds():
                    ...
                    self.request.response.setStatus(400)
                    return {
                        "error": f"State '{new_state_id}' does not exist in workflow."
                    }
        ...
        try:
            ...
            if "trigger_type" in body:
                ...
                if trigger_type in [TRIGGER_AUTOMATIC, TRIGGER_USER_ACTION]:
                    ...
                    self.request.response.setStatus(400)
                    return {
                        "error": f"Invalid trigger_type. Must be {TRIGGER_AUTOMATIC} or {TRIGGER_USER_ACTION}."
                    }
